...
TEXTS = {
    "English": {
        ...
        "tagline": "Clear visuals, smart alerts, India-ready geography, and teacher-friendly premium design",
        "dashboard": "🏠 Dashboard",
...
def build_pdf_report(test_inputs, results, warnings_list, recs):
    buffer = io.BytesIO()
...
def show_water_test(assets, t):
    ...
    if submit:
        try:
            ...
            pdf_buffer = build_pdf_report(pdf_inputs, results, warnings_list, recs)
            st.download_button(
                t["export_pdf"],
                data=pdf_buffer,
...
def show_about(assets, t):
    ...
    st.write(
        ...
        health advisory cards, PDF reports, batch analysis, multilingual interface, and
        India-ready state selection with Maharashtra district support.
        """
    )
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.markdown(
            '<div class="small-note">Premium UI • Splash Screen • India States • Maharashtra Districts • Gauge Risk • PDF Export</div>',
            unsafe_allow_html=True,
        )
